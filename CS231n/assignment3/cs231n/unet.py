...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        x = (cfg_scale + 1) * self.forward(x, time, model_kwargs=model_kwargs) - cfg_scale * self.forward(x, time, model_kwargs={'text_emb': None})
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for i, down_block in enumerate(self.downs):
          for j, layer in enumerate(down_block):
            if j < 2:
        ...
        for i, up_block in enumerate(self.ups):
          for j, layer in enumerate(up_block):
            if j >= 1:
              x = layer(torch.concat([x, connection.pop()], dim=1), context=context)
            else:
              x = layer(x)
        ##################################################################
