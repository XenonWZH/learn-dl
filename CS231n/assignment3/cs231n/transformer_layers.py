...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        x = torch.arange(max_len).view(-1, 1) * (10000 ** (-torch.arange(0, embed_dim, 2) / embed_dim)).view(1, -1)
        pe[0, :, 0::2] = torch.sin(x)
        pe[0, :, 1::2] = torch.cos(x)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        H, E, D = self.n_head, self.emd_dim, self.head_dim
        ...
        e = torch.matmul(q, k.transpose(2, 3)) / math.sqrt(D)
        if attn_mask is not None:
          e = e.masked_fill(attn_mask == 0, float('-inf'))
        attn = self.attn_drop(torch.softmax(e, dim=-1))
