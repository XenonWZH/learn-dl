...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    similarity = (text_features @ image_features.T) / torch.norm(text_features, dim=1, keepdim=True) / torch.norm(image_features, dim=1, keepdim=True).T
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    image = [clip_preprocess(Image.fromarray(img)) for img in images]
    ...
    pred_classes = torch.argmax(get_similarity_no_loop(text_features, image_features), dim=0, keepdim=False)
    pred_classes = [class_texts[idx] for idx in pred_classes.tolist()]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.image_features = clip_model.encode_image(image)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        similarity = ((text_features / torch.norm(text_features, dim=1, keepdim=True)) @ self.image_features.T)[0]
...
class DINOSegmentation:
    ...
    def inference(self, X_test):
        ...
        pred_classes = self.model(X_test)
        pred_classes = torch.argmax(pred_classes, dim=1, keepdim=False)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return pred_classes