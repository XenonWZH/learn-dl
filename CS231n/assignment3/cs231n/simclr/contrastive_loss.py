...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        denominator = 0
        for i, z_i in enumerate(out):
          if i != k:
            denominator += torch.exp(sim(z_i, z_k) / tau)
        l_k_kN = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / denominator)

        denominator = 0
        for i, z_i in enumerate(out):
          if i != k + N:
            denominator += torch.exp(sim(z_i, z_k_N) / tau)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    similarity = torch.concat((sim_positive_pairs(out_left, out_right), sim_positive_pairs(out_right, out_left)), dim=0)
