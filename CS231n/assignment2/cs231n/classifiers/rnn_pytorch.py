...
class CaptioningRNN:
    ...
    def sample(self, features, max_length=30):
        ...
        x = torch.LongTensor([self._start for _ in range(N)])

        for i in range(max_length):
          embedded = word_embedding_forward(x.view(N, 1), W_embed).view(N, -1)
          if self.cell_type == 'rnn':
            h = rnn_step_forward(embedded, h, Wx, Wh, b)
          elif self.cell_type == 'lstm':
            pass
          scores = temporal_affine_forward(h.view(N, 1, -1), W_vocab, b_vocab).view(N, -1)
          x = torch.argmax(scores, dim=1)
          captions[:, i] = x
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
